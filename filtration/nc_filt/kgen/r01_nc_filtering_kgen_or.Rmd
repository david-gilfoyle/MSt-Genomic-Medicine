---
title: "r01_nc_filtering_kgen_or"
...
Variant filtering of AF filtered dataset to select non-coding (Consequence), high confidence (HC) non-coding (FATHMM_MKL_NC > 0.5 and CADD_PHRED > 15) and HC non-coding variants in genes with pLI >= 0.9  
...
base_folder="/home/ec2-user/scripts/filtration/nc_filt/kgen"
output_folder="/home/ec2-user/results/filtered/nc_filt/kgen"
...
2) CADD_PHRED > 15 - assessment indicates that some entries are NA - these appear to be mainly insertion, deletion and substitution variants and they will be retained. However, removal of variants with CADD_PHRED <= 15 is considered appropriate to enrich dataset for potentially deleterious variants
...
Deleterious non-coding variants in a single allele are expected to have a greater impact in loss intolerant genes (ExAC pLI >= 0.9) 
...
output_hc_pli.df$FATHMM_MKL_NC[1:20]
...
fixed_nc_filt_kgen_or.df <- output.df
fixed_nc_hc_filt_kgen_or.df <- output_hc.df
fixed_nc_hc_pli_filt_kgen_or.df <- output_hc_pli.df
gt_nc_filt_kgen_or.mx <- output.mx
gt_nc_hc_filt_kgen_or.mx <- output_hc.mx
gt_nc_hc_pli_filt_kgen_or.mx <- output_hc_pli.mx
...
save.image(paste(output_folder, "r01_kgen_or_nc_filtered.RData", sep="/"))
