...
vcf_file <- paste(input_folder,"s01_ddr_kgen_vep_annotated.vcf",sep="/")
...
fixed_kgen_ddr.df <- as.data.frame(meta_fix$fix)
dim(fixed_kgen_ddr.df)
fixed_kgen_ddr.df[1:20,1:10]

# extract genotypes matrix 
gt_kgen_ddr.mx <- extract.gt(vcfr) # original numeric codes: 0/1, 1/1 etc
NA -> gt_kgen_ddr.mx[gt_kgen_ddr.mx == "."] # assign NA to '.'
dim(gt_kgen_ddr.mx)
gt_kgen_ddr.mx[1:5,1:3]
...
fixed_kgen_ddr.df <- separate_(fixed_kgen_ddr.df, "CSQ", vep_fields, sep="\\|") 
...
NA -> fixed_kgen_ddr.df[fixed_kgen_ddr.df == ""]
dim(fixed_kgen_ddr.df)
fixed_kgen_ddr.df[1:5,80:85]
...
save.image(paste(output_folder, "r01_import_vcf_kgen_ddr.RData", sep="/"))
