---
title: "r01_nc_filtering_exac_full_ddr"
...
Variant filtering of AF filtered dataset to select non-coding (Consequence), high confidence (HC) non-coding (FATHMM_MKL_NC > 0.5 and CADD_PHRED > 15) and HC non-coding variants associated with genes with pLI >= 0.9  
Dataset: ExAC (full)  
...
fixed_nc_filt_exac_full_ddr.df <- output.df
fixed_nc_hc_filt_exac_full_ddr.df <- output_hc.df
fixed_nc_hc_pli_filt_exac_full_ddr.df <- output_hc_pli.df
...
save.image(paste(output_folder, "r01_exac_full_ddr_nc_filtered.RData", sep="/"))
