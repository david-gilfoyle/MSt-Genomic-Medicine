...
# s02_preprocessing_qc_icr1000_ddr.sh
...
# Use:
# ./s02_preprocessing_qc_icr1000_ddr.sh &> s02_preprocessing_qc_icr1000_ddr.log
...
nonmito_file="${base_folder}/resources/icr1000/1958BC_hg19_mitofree.vcf.gz"
...
# Assessment of Source VCF File - Excluding Mitochondrial Genomne
#_______________________________________________________________________

echo "Source VCF File (Excluding Mitochondrial Genome Variants):" "${nonmito_file}"

# Counting variants in VCF file

vars_no=$(zgrep -vc "^#" "${nonmito_file}")
echo "Num of variants:" $(printf "%'d" "${vars_no}")
  
# Counting samples in VCF file

header_line=$(zgrep -m 1 "^#CHROM" "${nonmito_file}")
columns_no=$(wc -w <<< "${header_line}")
samples_no=$(( ${columns_no} - 9 ))
echo "Num of samples:" $(printf "%'d" "${samples_no}")
echo ""
...
"${bcftools}" stats -F "${ref_genome}" "${nonmito_file}" > "${vcf_stats1}" 
