#!/bin/bash
# s01_sort_merge_NA19834_ddr_positive_kgen.sh

# Started: David Gilfoyle 27May2018
# Last updated: David Gilfoyle 27May2018

# Use:
# ./s01_sort_merge_NA19834_ddr_positive_kgen.sh &> s01_sort_merge_NA19834_ddr_positive_kgen.log

# Sorting and Remerging of VCF Files for Selected DDR Positive Individuals from 1000 Genome Project (Genotype VCF Files)

# The initial merging of VCF files from positive subject NA19834 failed during chromosome 2
# This script sorts the chromosome 2 VCF file before remerging
...
# Starting script
echo "Starting s01_sort_merge_NA19834_ddr_positive_kgen.sh"
date
echo ""
...
echo ""
echo "Sorting individual genotype VCF file (chromosome 2) from 1000 Genome Project"
date

java -jar "${picard_tool}"/picard.jar SortVcf \
	I="${input_folder}"/s01_${individual_id}_ddr_positive_kgen_chr2_raw.vcf \
	O="${input_folder}"/s01_${individual_id}_ddr_positive_kgen_chr2_raw_sorted.vcf

# Merging 1000 Genome Individual Genotype VCF files
echo ""
echo "Merging DDR control individual genotype VCF files (including sorted chromosome 2) from 1000 Genome Project"
date

java -jar "${picard_tool}"/picard.jar MergeVcfs \
I="${input_folder}"/s01_${individual_id}_ddr_positive_kgen_chr1_raw.vcf \
I="${input_folder}"/s01_${individual_id}_ddr_positive_kgen_chr2_raw_sorted.vcf \
...
# Finishing script
echo "Completed s01_sort_merge_NA19834_ddr_positive_kgen.sh"
date
echo ""
