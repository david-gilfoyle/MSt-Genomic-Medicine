---
title: "r01_reg_filtering_kgen_ddr"
...
Variant filtering of AF filtered dataset to select non-clinical (Consequence), high confidence (HC) non-clinical (FATHMM_MKL_NC > 0.5 and CADD_PHRED > 15) and HC non-clinical variants in genes with pLI >= 0.9  
Dataset: 1000 Genome Project  
Gene Selection: DNA Repair Genes  
Input Variants: 313,889
...
Deleterious non-clinical variants in a single allele are expected to have a greater impact in loss intolerant genes (ExAC pLI >= 0.9) 
...
fixed_nc_filt_kgen_ddr.df <- output.df
fixed_nc_hc_filt_kgen_ddr.df <- output_hc.df
fixed_nc_hc_pli_filt_kgen_ddr.df <- output_hc_pli.df
gt_nc_filt_kgen_ddr.mx <- output.mx
gt_nc_hc_filt_kgen_ddr.mx <- output_hc.mx
gt_nc_hc_pli_filt_kgen_ddr.mx <- output_hc_pli.mx

# Clean-up
rm(input.df, output.df, output_hc.df, output_hc_pli.df, input.mx, output.mx, output_hc.mx, output_hc_pli.mx, ddr_genes, removed_variant.df)
...
save.image(paste(output_folder, "r01_kgen_ddr_nc_filtered.RData", sep="/"))
