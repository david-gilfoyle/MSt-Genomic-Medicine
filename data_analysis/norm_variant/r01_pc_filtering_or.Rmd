...
Filtering of AF filtered dataset to select protein-coding (by Consequence) to support assessment of normalised protein-coding variants against total OR gene transcript length  
Dataset: All  
...
dim(fixed_af_filt_icr1000_or.df) # should be 2,539
dim(fixed_af_filt_kgen_or.df) # should be 47,482
...
dim(exac_full_or.df) # should be 55,000
dim(exac_nontcga_or.df) # should be 50,655
dim(icr1000_or.df) # should be 2,503
dim(kgen_or.df) # should be 42,006
...
exac_full_pc_or.df <- exac_full_or.df %>% 
...
exac_nontcga_pc_or.df <- exac_nontcga_or.df %>% 
  filter(grepl("synonymous_variant",Consequence) |
           grepl("missense_variant",Consequence) |
           grepl("inframe_insertion", Consequence) |
           grepl("inframe_deletion", Consequence) |
...
icr1000_pc_or.df <- icr1000_or.df %>% 
  filter(grepl("synonymous_variant",Consequence) |
           grepl("missense_variant",Consequence) |
           grepl("inframe_insertion", Consequence) |
           grepl("inframe_deletion", Consequence) |
...
unique(exac_full_pc_or.df$Consequence)
dim(exac_full_pc_or.df)
unique(exac_nontcga_pc_or.df$Consequence)
dim(exac_nontcga_pc_or.df)
unique(icr1000_pc_or.df$Consequence)
dim(icr1000_pc_or.df)
unique(kgen_pc_or.df$Consequence)
dim(kgen_pc_or.df)
