---
title: "r01_mut_sig_ddr_pos_NA19131_common"
...
(high-confidence LoF variants) against control individuals - common variants (AF>0.1)
Dataset: 1000 Genomes Project  
Individual: NA19131 (YRI) - DDR deficient (homozygous SLX1A and heterozygous ERCC1)
...
# Set sample name
sample_name <- c("NA19131_common variants")

# Set and load working VCF file
vcf_file="~/results/data_analysis/gt_individuals/kgen/ddr_positive/NA19131/s01_NA19131_ddr_positive_kgen_merged_raw.vcf.gz"
...
# common_variant_selection

AF reflects the individual, therefore, AFR_AF used for filtering 

```{r common_variant_selection}

# Check AF
all_fixed.df[1:10,1:20]
all_fixed.df$AFR_AF[1:10]

# Filter AF > 0.1
common_fixed.df <- all_fixed.df %>% 
  filter(as.numeric(AFR_AF) > 0.1)

# Check AF
common_fixed.df[1:10,1:20]
common_fixed.df$AFR_AF[1:10]
...
required_fixed.df <- common_fixed.df[,c("CHROM", "POS", "REF", "ALT")]
...
# Check dataframe
dim(required_fixed.df)   # 4,274,633 x 5
...
# Check dataframe
dim(required_fixed.df) # 3,734,055 x 5
...
save.image("~/results/data_analysis/mut_sig/r01_mut_sig_ddr_pos_NA19131_common.Rdata")
