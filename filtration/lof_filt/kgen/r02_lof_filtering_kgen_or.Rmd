---
title: "r02_lof_filtering_kgen_or"
...
Variant filtering of AF filtered dataset to select loss of function (LoF; Consequence), LoF with LOFTEE High Confidence (HC)  
and HC LoF variants in genes with pLI >= 0.9  
...
base_folder="/home/ec2-user/scripts/filtration/lof_filt/kgen"
output_folder="/home/ec2-user/results/filtered/lof_filt/kgen"
...
2) CADD_PHRED > 15 - assessment indicates that some entries are NA - these appear to be mainly missense (insertion and deletion variants) and they will be retained. However, removal of variants with CADD_PHRED <= 15 is considered appropriate to enrich dataset for potentially deleterious variants
...
# Assessment of filtering CADD_PHREDD > 15
sum(is.na(output.df$CADD_PHRED))
sum(as.numeric(output.df$CADD_PHRED) == 15, na.rm = TRUE)
sum(as.numeric(output.df$CADD_PHRED) < 15, na.rm = TRUE)
sum(as.numeric(output.df$CADD_PHRED) > 15, na.rm = TRUE)
...
# Additional filtration based on LOFTEE (high confidence) and CADD_PHRED > 15
output.df <- tibble::rownames_to_column(output.df, var = "rowname")
...
fixed_lof_filt_kgen_or.df <- output.df
fixed_lof_hc_filt_kgen_or.df <- output_hc.df
fixed_lof_hc_pli_filt_kgen_or.df <- output_hc_pli.df
gt_lof_filt_kgen_or.mx <- output.mx
gt_lof_hc_filt_kgen_or.mx <- output_hc.mx
gt_lof_hc_pli_filt_kgen_or.mx <- output_hc_pli.mx
...
save.image(paste(output_folder, "r02_kgen_or_lof_filtered.RData", sep="/"))
