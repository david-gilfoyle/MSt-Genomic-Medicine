---
title: "r01_mut_sig_ddr_pos_HG00310_intermediate"
...
Updated: David Gilfoyle 3 Jul 2018

---

# Summary

Assessment of mutational signatures for individuals with DDR deficiencies  
(high-confidence LoF variants) against control individuals - intermediate variants (AF>0.01 and <=0.1)
Dataset: 1000 Genomes Project  
Individual: HG00310 (FIN) - DDR deficient (heterozygous EXO1 and MLH3)
...
# Set sample name
sample_name <- c("HG00310_intermediate variants")

# Set and load working VCF file
vcf_file="~/results/data_analysis/gt_individuals/kgen/ddr_positive/HG00310/s01_HG00310_ddr_positive_kgen_merged_raw.vcf.gz"
...
# Get data from meta (header) and fixed columns (10-20 min!)
...
# intermediate_variant_selection

AF reflects the individual, therefore, EUR_AF used for filtering 

```{r rare_variant_selection}

# Check AF
all_fixed.df[1:10,1:20]
all_fixed.df$EUR_AF[1:10]

# Filter AF <= 0.01
intermediate_fixed.df <- all_fixed.df %>% 
  filter(as.numeric(EUR_AF) > 0.01) %>%
  filter(as.numeric(EUR_AF) <= 0.1)

# Check AF
intermediate_fixed.df[1:10,1:20]
intermediate_fixed.df$EUR_AF[1:100]
...
required_fixed.df <- intermediate_fixed.df[,c("CHROM", "POS", "REF", "ALT")]
...
# Check dataframe
dim(required_fixed.df)   # 49,989 x 5

# Clean-up
rm(intermediate_fixed.df)
...
# Check dataframe
dim(required_fixed.df) # 47,024 x 5
...
save.image("~/results/data_analysis/mut_sig/r01_mut_sig_ddr_pos_HG00310_intermediate.Rdata")
